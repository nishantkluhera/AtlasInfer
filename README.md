...
Two measured claims, on **current models** (Qwen3, 2025; Qwen2.5, 2024):

1. **It's competitive with bitsandbytes**, the standard accessible-quant library:
   AtlasInfer's INT8 is lossless and edges LLM.int8(), and its 4-bit NF4 *beats*
   bitsandbytes' NF4 on perplexity (see [vs bitsandbytes](#vs-bitsandbytes)).
2. **Per-layer mixed precision beats any uniform bit-width** at a given memory
   footprint — a knob bitsandbytes doesn't have.

On **Qwen3-0.6B**, uniform 4-bit costs **+1.34 perplexity**; spending a little
more (~4.5 bits, per-layer) cuts that to **+0.88**, and INT8 is effectively free
(**+0.01**). The blue mixed-precision curve sits strictly below the uniform line:
...
Qwen2.5-0.5B, decoding 64 tokens on an RTX 3060
(`python bench_latency.py --model Qwen/Qwen2.5-0.5B`):

| Config | Peak GPU (MB) | vs FP16 | tok/s | rel. speed |
| --- | ---: | ---: | ---: | ---: |
| fp16 | 960.7 | 1.00x | 16.9 | 1.00x |
| uniform-int8 | 672.2 | 0.70x | 10.4 | 0.61x |
| uniform-nf4 | 571.9 | 0.60x | 5.2 | 0.31x |
| mixed-5bit | 607.4 | 0.63x | 6.0 | 0.35x |

So the trade is **30–40% less peak GPU memory for slower decode** (≈1.6x for INT8,
≈3x for NF4 — the codebook dequant is the most expensive eager path) — the right
...
(`python compare_baselines.py`), across **current models and a 4x-larger one** to
check the result isn't a single-model fluke — the same conclusions hold on all
three (delta vs FP16 perplexity / weights MB):

**Qwen3-0.6B** (2025, the most current):
| Method | ~bits | MB | delta ppl |
| --- | ---: | ---: | ---: |
| AtlasInfer int8 | 8 | 738.8 | **+0.009** |
| bnb int8 (LLM.int8) | 8 | 716.9 | +0.057 |
| AtlasInfer mixed-5bit | 5 | 608.2 | **+0.722** |
| AtlasInfer nf4 | 4 | 564.7 | **+1.339** |
| bnb nf4 | 4 | 506.9 | +1.834 |

**Qwen2.5-0.5B** (2024):
| Method | ~bits | MB | delta ppl |
| --- | ---: | ---: | ---: |
| AtlasInfer int8 | 8 | 620.5 | **+0.007** |
| bnb int8 (LLM.int8) | 8 | 601.0 | +0.070 |
| AtlasInfer mixed-5bit | 5 | 515.1 | **+0.509** |
| AtlasInfer nf4 | 4 | 479.6 | **+0.943** |
| bnb nf4 | 4 | 430.4 | +1.323 |

**Pythia-1.4B** (older arch, 2.3x larger — checks scale):
| Method | ~bits | MB | delta ppl |
| --- | ---: | ---: | ---: |
| AtlasInfer int8 | 8 | 1601 | **-0.005** |
| bnb int8 (LLM.int8) | 8 | 1546 | +0.056 |
| AtlasInfer mixed-5bit | 5 | 1237 | **+0.526** |
| AtlasInfer nf4 | 4 | 1118 | **+0.754** |
| bnb nf4 | 4 | 970 | +0.922 |

On every model: AtlasInfer's **INT8 matches/beats** bitsandbytes' LLM.int8();
AtlasInfer's **NF4 beats** bnb's NF4 on perplexity (NormalFloat codebook + sparse
outliers); and **mixed precision is the differentiator** — best accuracy of any
sub-8-bit option, because it spends bits per-layer (a knob bnb doesn't expose).
The one axis where bnb wins is 4-bit *memory* (~10% smaller — it double-quantizes
its scales; a roadmap item). Full notes:
...
- **Done:** NF4 codebook for the 4-bit tier — beats bitsandbytes' NF4 on
  perplexity (see comparison above).
- **Done:** fused W8A16 **and** W4A16 Triton kernels (1.4–1.95x over FP16 at
  decode), wired into `AtlasInference` (`kernel="auto"`).
- Double-quantize the block scales (as bitsandbytes does) to close AtlasInfer's
  remaining ~10% memory gap at 4-bit.
